...
func MakeServer(port int, protocol network.Protocol, tlsConfig *tls.Config) *Server {
	s := new(Server)
	s.port = port
	s.proto = protocol
	s.conns = sync.Map{}
...
func (s *Server) Start() (error, chan interface{}) {
	listener, err := quic.ListenAddr(":"+strconv.Itoa(s.port), s.tlsConfig, &quic.Config{MaxIncomingStreams: 1 << 10})
	if err != nil {
	...
	go func() {
		for {
			conn, acceptErr := s.listener.Accept(context.Background())
			if acceptErr != nil {
				break
			}
			stream, acceptErr := conn.AcceptStream(context.Background())
			if acceptErr != nil {
				log.Println("open quic stream error ", err)
				continue
			}
			// accepted connection, generate conn ID and bind protocol
			connection := MakeQuicConnection(s.proto, atomic.AddUint32(&s.nextConnId, 1), s.router, stream, false)
	...
	log.Println("QUIC server started, listening: ", s.port)
